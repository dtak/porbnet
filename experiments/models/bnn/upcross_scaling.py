...
dim_hidden_list = np.array([1000.])
prior_w1_sig2_list = np.array([100., 1000, 10000, 20000])
prior_w2_sig2_list = np.array([0.7, 1.4, 2.8])
	...
	for j, prior_w1_sig2 in enumerate(prior_w1_sig2_list):
		for k, prior_w2_sig2 in enumerate(prior_w2_sig2_list):
			...
			net = networks.BNN(dim_in=1, dim_hidden=int(dim_hidden), dim_out=1, \
					prior_w1_sig2 = prior_w1_sig2, prior_b1_sig2 = 10., \
...
# color: prior_w2_sig2
# fixed: dim_hidden
ax[0,0].plot(prior_w1_sig2_list, np.squeeze(upcross[0,:,:]),'-o')
...
ax[0,0].legend(prior_w2_sig2_list, title=r'$\sigma^2_{w2}$',loc='center left', bbox_to_anchor=(1, 0.5))
...
# color: prior_w1_sig2
# fixed: dim_hidden
ax[0,1].plot(prior_w2_sig2_list, np.squeeze(upcross[0,:,:]).T,'-o')
...
ax[0,1].legend(prior_w1_sig2_list, title=r'$\sigma^2_{w1}$',loc='center left', bbox_to_anchor=(1, 0.5))

# y: var
# x: prior_w1_sig2
# color: prior_w2_sig2
# fixed: dim_hidden
ax[1,0].plot(prior_w1_sig2_list, np.squeeze(variance[0,:,:]),'-o')
...
ax[1,0].legend(prior_w2_sig2_list, title=r'$\sigma^2_{w2}$',loc='center left', bbox_to_anchor=(1, 0.5))

# y: var
# x: prior_w2_sig2
# color: prior_w1_sig2
# fixed: dim_hidden
ax[1,1].plot(prior_w2_sig2_list, np.squeeze(variance[0,:,:]).T,'-o')
...
ax[1,1].legend(prior_w1_sig2_list, title=r'$\sigma^2_{w1}$',loc='center left', bbox_to_anchor=(1, 0.5))
